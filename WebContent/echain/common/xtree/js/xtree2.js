...
_p.getRowHtml = function () {
	var t = this.getTree();
	var str= "<div class=\"" + this.getRowClassName() + "\" style=\"padding-left:" +
		...
		if( this.iptType == "1" )
		{
			if( this.checked )
			{ 
				str += "<input type=\"radio\" name=\"checkbox_tree\" value=\"" + this.value + "\" checked class=\"webfx-tree-iptType\" />";
			}
			else
			{
				str += "<input type=\"radio\" name=\"checkbox_tree\" value=\"" + this.value + "\" class=\"webfx-tree-iptType\" />  ";
			}
		}else if( this.iptType == "n" ){
			if( this.checked )
			{ 
				str += "<input type=\"checkbox\" name=\"checkbox_tree\" value=\"" + this.value + "\" checked class=\"webfx-tree-iptType\"/>";
			}
			else
			{
				str += "<input type=\"checkbox\" name=\"checkbox_tree\" value=\"" + this.value + "\" class=\"webfx-tree-iptType\"/>  ";
			}
		}
        str=str+this.getLabelHtml() +
		"</div>";
		return str;
};
...
function WebFXTreeItem(sText, oAction, eParent, sIcon, sOpenIcon,value, iptType, checked) {
	WebFXTreeAbstractNode.call(this, sText, oAction);
	if (sIcon) this.icon = sIcon;
	if (sOpenIcon) this.openIcon = sOpenIcon;
	...
	if(iptType)this.iptType = iptType;
