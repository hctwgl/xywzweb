<!--
document.write("<div id=meizzCalendarLayer style='position: absolute; z-index: 9999; width: 150; height: 300; display: none'>");
...
{    //parent.document.write("11111")
	...
	"	 <tr  class=bg style='"+WebCalendar.isTime+"'>"+
     "<select name=tmpHourSelect onblur='parent.hiddenSelect(this)' style='z-index:2;position:absolute;left:3;display:none'"+
    " onchange='parent.WebCalendar.thisHour=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
	 "<select name=tmpMinuteSelect onblur='parent.hiddenSelect(this)' style='z-index:2;position:absolute;left:55;display:none'"+
    " onchange='parent.WebCalendar.thisMinute=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
	 "<select name=tmpSecondSelect onblur='parent.hiddenSelect(this)' style='z-index:2;position:absolute;right:8;display:none'"+
    " onchange='parent.WebCalendar.thisSecond=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "    <td width=40 id=meizzHour title='点击此处选择时' onclick='parent.funHourSelect()'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'></td>"+	
    "    <td width=50 align=center id=meizzMinute title='点击此处选择分' onclick='parent.funMinuteSelect()'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'></td>"+
	"    <td width=40  align=right id=meizzSecond title='点击此处选择秒' onclick='parent.funSecondSelect()'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'></td>"+
   "</tr>"+
    ...
    "    onfocus='this.blur()' style='meizz:expression(this.disabled=parent.WebCalendar.thisYear==9999)'></td></tr>"+
	" </table>"+
   "</td></tr><table>"+	
	"</form></body></html>";
...
function calendar() //主调函数
{
	...
	if(WebCalendar.timeShow)
		WebCalendar.isTime ="display:";
	else
		WebCalendar.isTime ="display:none";
	writeIframe();
    ...
    try{
        ...
        if (WebCalendar.objExport.value.trim() != ""){
            ...
            if (WebCalendar.dateStyle == null)
            {
				...
				if(m<=9)m="0"+m;
				if(s<=9)s="0"+s;
                WebCalendar.thisHour = h;
				WebCalendar.thisMinute = m;
				WebCalendar.thisSecond = s;
...
function funHourSelect() //时的下拉框
{
	...
	for (var i=0; i<=23; i++){
	   ...
	   var oOption = WebCalendar.iframe.document.createElement("OPTION");
	  
	   e.options.add(oOption);oOption.innerText=h ;
	   oOption.value=h
		 h++;  
	   };
...
function funMinuteSelect() //时的下拉框
{
   ...
   for (var i=0; i<=59; i++){
	  ...
	  if(m<=9)
		   m = "0"+m;
	   e.options.add(oOption);oOption.innerText=m ;
	   oOption.value=m
		  m++; 
	   };
...
function funSecondSelect() //时的下拉框
{
   ...
   for (var i=0; i<=59; i++){
	   ...
	   e.options.add(oOption);oOption.innerText=s ;
	   oOption.value=s
		 s++;  };
...
{	
   var s = WebCalendar.thisSecond;
    ...
    WebCalendar.iframe.meizzHour.innerText = " "+h +" 时";
	WebCalendar.iframe.meizzMinute.innerText = "  "+mm +" 分";
	WebCalendar.iframe.meizzSecond.innerText = " "+s +" 秒";
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
		...
		//d = WebCalendar.format.match(/^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/);
        if(d==null){alert("你设定的日期输出格式不对！\r\n\r\n请重新定义 WebCalendar.format ！"); return false;}
        var flag = d[3].length==2 || d[4].length==2; //判断返回的日期格式是否要补零
        returnValue = flag ? a[2] +d[2]+ appendZero(a[1]) +d[2]+ appendZero(a[0]) : a[2] +d[2]+ a[1] +d[2]+ a[0];
		if(WebCalendar.timeShow)
		returnValue += " "+WebCalendar.thisHour+":"+WebCalendar.thisMinute+":"+WebCalendar.thisSecond;
