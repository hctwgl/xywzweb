...
Vector vect=new Entrust().queryAgentLog(userid);
%>
<html>
<head>
...
<%
	...
	for (int i=0;i<vect.size();i++) {
		...
		nodeid=(String)vecRow.elementAt(4);
	    if(tr){
%>
...
%>
<td><a href='<%=request.getContextPath()%>/echain/flowdemo/form_main.jsp?instanceid=<%=instanceid%>&nodeid=<%=nodeid%>'><%=(String)vecRow.elementAt(1)%></a></td>
<td><%=(String)vecRow.elementAt(2)%></td>
<td><%=(String)vecRow.elementAt(3)%></td>
<td><%=(String)vecRow.elementAt(5)%></td>
<td><%=(String)vecRow.elementAt(6)%></td>
</tr>
<%
		tr=!tr;
	}
