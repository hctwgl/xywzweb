...
<%@ page import="com.ecc.echain.forms.*"%>
<%@ page import="com.ecc.echain.forms.model.*"%>
<%
...
if(orgid==null||orgid.equals("")||orgid.equals("null"))
	orgid="default";
Map hm=FormFactory.getInstance().getFormClass().getTopResourceModel(orgid);
ResourceModel rm;
%>
<html>
<head>
...
function retSuc(){
	    ...
	    for (var i = 0; i < attribute.length; i++){
	         if(!attribute[i].checked)
				continue;
	         if(orgid==null){
				orgid= attribute[i].value.substring(0,attribute[i].value.indexOf("@"));
				orgname= attribute[i].value.substring(attribute[i].value.indexOf("@")+1,attribute[i].value.length);
			 }
	         else{
				orgid+= ";"+attribute[i].value.substring(0,attribute[i].value.indexOf("@"));
				orgname+= ";"+attribute[i].value.substring(attribute[i].value.indexOf("@")+1,attribute[i].value.length);
...
<div id="leftMenuBar" style="height:470px; width:350; overflow: auto; text-align: left;">
	  ...
	  var path='<%=request.getContextPath()%>/echain/common/xtree/images/';
       ...
       var tree = new WebFXTree('请选择业务品种');
    <% 
    	Iterator it=hm.keySet().iterator();
       while(it.hasNext()){
			rm=(ResourceModel)hm.get((String)it.next());
			out.print("tree.add(new WebFXLoadTreeItem('"+rm.getName()+"','"+request.getContextPath()+"/echaincommonservlet?method=echaincommon&actionType=selectapp&appid="+rm.getId()+"&count="+count+"',null,null,null,null,'"+rm.getId()+"@"+rm.getName()+"','"+count+"',null));");	
        }
    %>
          tree.indentWidth = 19;
