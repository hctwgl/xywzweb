...
(function($) {
	...
	var defaults = {
			...
			onDayCellClick: function(date) { return false; },
			onDayCellDblClick: function(dateIn) { return false; },
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				var dateBox = jQuery("<td></td>").attr(atts).append(dateLink).dblclick(function(e) {
					$(".DateBox").removeClass("dbSelected");
		...
		var a = jQuery(ids.container).css({ "width" : defaults.width + "px", "height" : defaults.height + "px","display": 'block'});
	...
	var DrawEventsOnCalendar = function() {
		...
		/////////////
				if(ev.Time) {
					if (typeof ev.Time == 'object' && ev.Time.getDate) { sT = ev.Time; }
					if (typeof ev.Time == 'string' && ev.Time.split) { sT = GetJSONDate(ev.Time); }
				} else if(ev.Date) { // DEPRECATED
					if (typeof ev.Date == 'object' && ev.Date.getDate) { sT = ev.Date; }
					if (typeof ev.Date == 'string' && ev.Date.split) { sT = GetJSONDate(ev.Date); }
				} else {
					return;  //no start date, or legacy date. no event.
				}
		////////////
				...
				//while(eDt.getFullYear() > sDt.getFullYear() || eDt.getMonth() > sDt.getMonth() || eDt.getDate() >= sDt.getDate()){
				...
				if(sT) {
					if ((sT >= _beginDate) && (sT <= _endDate)) {			
						var cell = jQuery("#" + getDateId(sT), jQuery(ids.container));//cell.html('');
						var label = jQuery(".DateLabel", cell);
						/******************************************************
						//-->> 2014-02-19 liuch -- title is too long... -->>
						//*****************************************************/
						var includeChinese = function(s) {   
							if(/[^\x00-\xff]/g.test(s)) {
								return true;
							}   
							return false;
						};
						var showLength = 14; // 这个值最好能算出来。
						if (ev.Title && includeChinese(ev.Title)) {
							showLength = 7;
						}
						var showTitle = ev.Title;
						if (ev.Title && ev.Title.length > showLength) {
							showTitle = ev.Title.substring(0, showLength) + "...";
						}
						var link = jQuery('<a  title="' + ev.Title + '">' + showTitle + '</a>');
						...
						//-->> 2014-02-19 liuch -- events more... -->>
						//*****************************************************/
						if (!showEvtDate || (showEvtDate 
								&& (showEvtDate.getFullYear() != sT.getFullYear()  
										|| showEvtDate.getMonth() != sT.getMonth()  
										|| showEvtDate.getDate() != sT.getDate()))  ) {

							showEvtDate = sT;
							
							evtsCount = 0;
						}
						if (evtsCount == showEvtsCount + 1) {
							cell.append('<div class="Event"><a title="更多消息">更多...</a></div>');
							evtsCount ++ ;
						}
						if (evtsCount > showEvtsCount) {
	...
	/******************************************************************
	//-->> 2014-02-19 liuch 
	// -- clear the private-array which named calendarEvents... -->>
	//*****************************************************************/
	jQuery.J.ShowEvents = function(eventCollection) {
		if(eventCollection) {
			//alert(eventCollection.length);
