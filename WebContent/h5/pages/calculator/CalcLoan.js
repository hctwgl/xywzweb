...
{
	$("#tbody").html("");
	var LastSum=document.getElementById("edLastSum").value;
	var BeginYear=0;
	var LoanTimes=parseInt(document.getElementById("edTimes").value);
	var Rate=parseFloat(document.getElementById("edRate").value)/100;
	var floatRatio=parseFloat(document.getElementById("floatRatio").value)/100;
	
	Rate=Rate*(1+floatRatio);
	
	var PayWay=document.getElementById("ddlPayWay").value;
	if (document.getElementById("rbPayFreq_0").checked)
		PayFreq=1;
	else
	{
		if (document.getElementById("rbPayFreq_1").checked)
			PayFreq=3;
		else
			PayFreq=1;
	}
...
//	}
	...
	for (i=0;i<times;i++) 
	{
		...
		var li = '<tr>'
			+ '<td align="left"><span dataFld="Times">'+mRecTimes+'</span></td>'
			+ '<td align="left"><span dataFld="Year">'+mRecYear.getFullYear() + "-" + (mRecYear.getMonth() + 1) + "-" + mRecYear.getDate()+'</span></td>'
			+ '<td align="right"><span dataFld="RateSum">'+(Math.round(mRecRateSum * 100) / 100).toString()+'</span></td>'
			+ '<td align="right"><span dataFld="Corpus">'+Math.round(mRecCorpus * 100) / 100+'</span></td>'
			+ '<td align="right"><span dataFld="CorpusRate">'+Math.round((mRecRateSum + mRecCorpus) * 100) / 100+'</span></td>'
			+ '<td align="right"><span dataFld="LeavCorpus">'+Math.round(mRecLeavCorpus * 100) / 100+'</span></td>'
			+ '</tr>'
			;
		$("#tbody").append($(li));
		fTotalInterest += mRecRateSum;
		fTotalSum += mRecCorpus;
...
//		node.lastChild.text = Math.round(mRecLeavCorpus * 100) / 100;		
	...
	var li = '<tr>'
		+ '<td align="left"><span dataFld="Times">合计</span></td>'
		+ '<td align="left"><span dataFld="Year"></span></td>'
		+ '<td align="right"><span dataFld="RateSum">'+(Math.round(fTotalInterest * 100) / 100).toString()+'</span></td>'
		+ '<td align="right"><span dataFld="Corpus">'+Math.round(fTotalSum * 100) / 100+'</span></td>'
		+ '<td align="right"><span dataFld="CorpusRate">'+Math.round((fTotalInterest + fTotalSum) * 100) / 100+'</span></td>'
...
function ECorpus(LastSum,BeginYear,times,Rate,Freq)
{
	var r,Inter,c;
	var i=0,result=0;
...
//	}
	...
	for (i=0;i<times;i++)
	{
		...
		// 剩余的本金LeavCorpus		
		var li = '<tr>'
			+ '<td align="left"><span dataFld="Times">'+RecTimes+'</span></td>'
			+ '<td align="left"><span dataFld="Year">'+RecYear.getFullYear() + "-" + (RecYear.getMonth() + 1) + "-" + RecYear.getDate()+'</span></td>'
			+ '<td align="right"><span dataFld="RateSum">'+(Math.round(RecRateSum * 100) / 100).toString()+'</span></td>'
			+ '<td align="right"><span dataFld="Corpus">'+Math.round(RecCorpus * 100) / 100+'</span></td>'
			+ '<td align="right"><span dataFld="CorpusRate">'+Math.round((RecRateSum + RecCorpus) * 100) / 100+'</span></td>'
			+ '<td align="right"><span dataFld="LeavCorpus">'+Math.round(RecLeavCorpus * 100) / 100+'</span></td>'
			+ '</tr>'
			;
		$("#tbody").append($(li));
		fTotalInterest += RecRateSum;
		fTotalSum += RecCorpus;
...
function CalcPayOnce(LastSum,BeginYear,EndYear,Rate)
{
	var result=0;
	var r;
	var days;
