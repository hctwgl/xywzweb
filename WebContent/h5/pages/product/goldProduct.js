...
var tabPanelCfg = { tabItems :[{
                            	    ...
                            	    url     : basePath+'goldProductQueryAction.json?&goldId=1', //查询URL
...
//                            				mesUtil.alert('无查询结果！');
                               				...
                               				for (var i = 0; i < response.json.data.length; i++) {
                               					...
                               					var li  = '<div class="bQ" onclick="goTogoldDetailPage('+aResults[i].ID+')">'
                               						+'    <div class="bQL1">'
                               						+'        <p class="bQL1_sp">'
//                               						+ja
                               						+'        </p>'
                               						+'        <p class="bQL1Con">'+aResults[i].MATERIAL_+'</p>'
                               						+'    </div>'
                               						+'    <div class="bQL2">'
//                               						+'		  <p></br></p>'
...
//                               						+'        <p class="bQL2P2" style="text-align:center">' + aResults[i].PROD_NO+'</p>'
                               					...
                               					if(i+1 < response.json.data.length) {
                               						i++;
                               						li += '<div class="bQ" onclick="goTogoldDetailPage('+aResults[i].ID+')">'
                               							+'    <div class="bQL1">'
                               							+'        <p class="bQL1_sp">'
//                               							+ja
                               							+'        </p>'
...
//                               							+'        <p class="bQL2P2" style="text-align:center">' + aResults[i].PROD_NO+'</p>'
                            	    ...
                            	    url     : basePath+'goldProductQueryAction.json?&goldId=2', //查询URL
                            	    success : function(response){
                            			var aResults = response.json.data;
 ...
 * 页面初始化方法
 */
var goldTabPanel = null;
function load() {
	var goldTabPanel = TabPanelFactory.get(pageConfig);//tabs组件初始化
	goldTabPanel.loadTabItems();//tabs组件中页签分页组件初始化
	goldTabPanel.init();		 //tabs组件切换页签事件初始化
	goldTabPanel.activeTab(0);  //tabs组件显示第一个页签
	$("#searchPanelButton").bind("click", function() {
		goldTabPanel.refresh();
	});
}
/**
 * 页面分页绑定
 ...
 * @param id
 */
function goTogoldDetailPage(id) {
	goPage('goldProductDetail.html?id='+id);
}
