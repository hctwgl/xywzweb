    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			var $fileInput = $(this);
			var $fileInputs = $(".jhbb_publish_list");
...
//				window.lengths=files.length;
//				window.lengths = files.length;
				$(".dialog_uppic").show();
				for(var i = 0;i<files.length;i++){
					createBox( $fileInputs, files[i] ,webUploader);
				}
			});
			//绑定文件加入队列事件;
...
//			});
			...
			webUploader.on('uploadSuccess',function( file, response ){
				var file_id = file.id;
				var $parentFileBox = $fileInputs.next('.parentFileBox');
				...
				if ( $parentFileBox.length <= 0 ) {
					...
					$fileInputs.after( div );
					$parentFileBox = $fileInputs.next('.parentFileBox');
				
				}
				var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					<div class="viewThumb" dataurl="'+response.url+'"></div> \
				...
				var len=$parentFileBox.children('.fileBoxUl').find("li").length;
				if(len<9){
					var hei=$(".jhbb_detail_pic").height();
					$(".jhbb_detail_pic").show();
					$(".jhbb_detail_pic em").eq(0).html(len+1);
					$(".jhbb_detail_pic em").eq(1).html(9-(len+1));
				...
				//父容器宽度;
				var le=parseInt($(".parentFileBox").css("paddingLeft"));
				var wid=Math.round($(".parentFileBox>.fileBoxUl>li").outerWidth(true));
				var $width = $('.fileBoxUl>li').length * (wid+le*2);
				...
				var $diyCancel = $fileBox.children('.diyCancel').one('click',function(){
					...
					if(num<0){
						num=0
						$(".jhbb_detail_pic em").eq(1).html(9);
						$(".jhbb_detail_pic").hide();
					}else{
						var nums=parseInt($(".jhbb_detail_pic em").eq(1).html());
						$(".jhbb_detail_pic em").eq(1).html(nums+1);
					}		
					$(".jhbb_detail_pic em").eq(0).html(num);
				});
				...
				/*webUploader.makeThumb( file, function( error, dataSrc ) {
					if ( !error ) {*/
						$fileBox.find('.viewThumb').css({"background":"url("+response.url+") no-repeat center","background-size":"cover"} );
					/*}
				});*/
				var $fileBox = $('#fileBox_'+file.id);
				var $diyBar = $fileBox.find('.diyBar');	
...
//				for(var i=1;i<=window.lengths;i++){
//					if(i==window.lengths){
//						$(".dialog_uppic").hide();
//					}
//				}
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			fileNumLimit:9,
			fileSizeLimit:8000 * 1536,
			fileSingleSizeLimit:2048 * 1536
		};
	}
