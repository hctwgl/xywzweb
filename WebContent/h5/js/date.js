...
(function ($) {      
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        if(opts.theme === "datetimeap"){datetimeap = true;}
        if(opts.theme === "name"){name = true;}
        if(!opts.show){
            that.unbind('click');
        }
        else{
    ...
    function resetIndex(){
        ...
        function resetInitDete(){
            ...
            initY = parseInt(that.val().substr(2,2))-16;
            ...
            $("#dateconfirm").unbind('click').click(function () {   
              ...
              $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               ...
               if(datetime){
                    ...
                    var num=indexS+1;
                    datestr=$("#Secondwrapper ul li:eq("+num+")").html()+","+$("#Secondwrapper ul li:eq("+indexS+1+")").attr("datatime");
                    indexS=0;
                }else if(datetimeap){
                    if($("#Secondwrapper ul li:eq("+indexS+")").html()=="&nbsp;" || indexS==0){
                        datestr+="  上午";
                    }else{
                        //alert(indexS);
                        datestr+="  下午";
        ...
        function extendOptions(){
            $("#datePage").show(); 
            $("#datePage").css("marginTop",-parseFloat($("#datePage").height()/2))+"px"; 
            $("#dateshadow").show();
        }
        //日期滑动
        ...
        function names(){
            ...
            $("#datescroll").hide();
            $("#datescroll_datetime").show(); 
            $("#Secondwrapper ul").html(createSECOND_name());
        }
        ...
        function init_iScroll_datetime(){
         ...
         //日期+时间滑动
        function init_iScroll_AP(){
            SecondScroll = new iScroll("Secondwrapper",{snap:"li",vScrollbar:false,
                onScrollEnd:function () {
                    indexS = this.y;//Math.round((this.y/40)*(-1));
            }})
        } 
         //name
        function init_iScroll_name(){
            SecondScroll = new iScroll("Secondwrapper",{snap:"li",vScrollbar:false,
                onScrollEnd:function () {
                    indexS = Math.round((this.y/40)*(-1));
            }})
        ...
        function timesStyle(){
            ...
            $("#daywrapper").css("position","absolute");
            $("#daywrapper").css("bottom","200px");
            $("#Secondwrapper").css("marginLeft","-16%");
        }
        function nameStyle(){
            $("#datePage").css("height","245px");
            $("#datePage").css("top","60px");
            $("#datePage").css("marginTop","0");
            $("#yearwrapper").css("position","absolute");
            $("#yearwrapper").css("bottom","200px");
            ...
            $("#monthwrapper").css("bottom","200px");
            $("#daywrapper").css("position","absolute");
            $("#datescroll_datetime div").css("marginLeft",0);
            $("#daywrapper").css("bottom","200px");
            $("#Secondwrapper").css("marginLeft","-50%");
            $("#Secondwrapper").css("top","60px");
            $("#Secondwrapper").css("width","100%");
            $("#Secondwrapper ul").css("width","100%");
        }
        //创建 --年-- 列表
        ...
        //创建 name
        function createSECOND_name(){
            var str="<li>&nbsp;</li>";
            console.log(opts.data);
            var data=opts.data;
            $.each(data,function(index,val){
                str+='<li datatime="'+val.datetime+'">'+val.displaydatetime+'</li>';
            })
            return str+"<li>&nbsp;</li>";
        }
        //创建 --月-- 列表
