...
Mis.Ext.CrudPanel = Ext.extend(Ext.Panel,{
	...
	afterRender : function() {
		...
		var selectFormConf = {
				...
				split : true,
				buttons : [
				           {id:'commonExtPanelForQuery',text : '查询',handler : function() {this.stUrl = this.ownerCt.ownerCt.ownerCt.stUrl;this.ownerCt.ownerCt.ownerCt.loadCurrData();}},
				           {id:'commonExtPanelForReset',text : '重置',handler : function() {this.ownerCt.ownerCt.ownerCt.selectForm.getForm().reset();}} 
				           ]};
		if (this.selectItems)
        {
...
Ext.apply(util.layout,{
	_tr:function(){
		...
		for(var i=0;i<args.length;i++){debugger;res.items.push({columnWidth:args[i][0].columnWidth?args[i][0].columnWidth:(1.0/args.length),layout:'form',items:args[i]});}
		return res;
	}
});
Ext.apply(util.form,{
