...
(
	...
	Ext.form.TwinTriggerField,{
		initComponent : function(){
			Com.yucheng.crm.common.IndexField.superclass.initComponent.call(this);
		...
		onRender : function(ct, position){
			Com.yucheng.crm.common.IndexField.superclass.onRender.call(this, ct, position);
			if(this.hiddenName){
				var ownerForm = this;
				while(ownerForm.ownerCt && !Ext.instanceOf(ownerForm.ownerCt,'form'))
				{				//根据条件查询放大镜控件的最外层容器
		...
		onTrigger2Click : function()
		{
			...
			_this.orgUserPanel = new Ext.form.FormPanel
			(
				{//查询panel
					width:'25%',
					height:150,
			...
			(
				[//gridtable中的列定义
				    ...
				    {header:'用户ID',dataIndex:'userId',id:"userId",width:100,sortable : true},
				    ...
				    {header:'角色',dataIndex:'role',id:'role',width:130,hidden:false,sortable : true}
				]
			);
...
//						var __custZhNameStr='';
    					...
    					if(''!=searchField.hiddenField){
    						checkedNodes = _this.orgUserManageGrid.getSelectionModel().selections.items;
						if(searchField.singleSelected&&checkedNodes.length>1){
							Ext.Msg.alert('提示', '您只能选择一个客户群');
							return ;
						}else if(searchField.singleSelected&&checkedNodes.length<1){
...
//								searchField.hiddenField.setValue(checkedNodes[i].data.ID);
							}else{
								sName=sName+','+checkedNodes[i].data.NAME;
//								searchField.hiddenField.setValue(_this.hiddenField.value+','+checkedNodes[i].data.ID);
							}
						}
						searchField.setRawValue(sName);
						if(checkedNodes.length==1){
							//如果单选，则设置该客户相应的附属属性
							searchField.ID=checkedNodes[0].data.ID;//客户群ID
							searchField.NAME=checkedNodes[0].data.NAME;//客户群编号
							searchField.CONTENT=checkedNodes[0].data.CONTENT;//客户群编号
//							searchField.custBaseName=checkedNodes[0].data.CUST_BASE_NAME;//客户群名称
//							searchField.custStr=__custIdStr;//客户群成员编号
//							searchField.custStr1=__custZhNameStr;//客户群成员名称
						}
					}
				}
