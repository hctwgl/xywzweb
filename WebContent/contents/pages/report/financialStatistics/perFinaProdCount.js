...
Ext.onReady(function() {
	...
	//发行币种
	 var sellCurrStore = new Ext.data.ArrayStore({
			fields:['key','value'],
		    data:[['1','RMB'],['2','外币']]
	});
...
/**********************************************************/
    ...
    function select(){
    	...
    	var sellCurr = Ext.getCmp("sellCurr").getValue();
    	...
    	if(start==''&& sellCurr!=''){
    		if(sellCurr=='1')
    		{   
    			Ext.getCmp("sellCurr").setValue('');
    			var url=basepath+'/reportJsp/showReport.jsp?raq=/report_6_S61.raq&sellCurr='+'RMB';
    		}
        	else if(sellCurr=='2'){
        		Ext.getCmp("sellCurr").setValue('');
        		var url=basepath+'/reportJsp/showReport.jsp?raq=/report_6_S61.raq&sellCurr='+'外币';
    		}
    	} 
    	else if(sellCurr==''&& start!=''){
    		Ext.getCmp("fillTime").setValue('');
			var url=basepath+'/reportJsp/showReport.jsp?raq=/report_6_S61.raq&fillTime='+bDate;
    	}
    	else{
    ...
    var viewport = new Ext.Panel({
    	title:'理财报表统计->附件银行个人理财产品->商业银行个人理财产品统计表',
