...
/**********************************************************/
    ...
    function select(){
    	...
    	if(start==''&& sellCurr!=''){
    		...
    		{   
    			...
    			var url=basepath+'/reportJsp/showReport.jsp?raq=/report_4.raq&sellCurr='+'RMB';
    		}
        	else if(sellCurr=='2'){
        		Ext.getCmp("sellCurr").setValue('');
        		var url=basepath+'/reportJsp/showReport.jsp?raq=/report_4.raq&sellCurr='+'外币';
    		}
    	} 
    	else if(sellCurr==''&& start!=''){
    		Ext.getCmp("fillTime").setValue('');
			var url=basepath+'/reportJsp/showReport.jsp?raq=/report_4.raq&fillTime='+bDate;
    	}
    	else{
    ...
    var viewport = new Ext.Panel({
    	title:'理财报表统计->附件银行个人理财产品->商业银行个人理财产品调查表',
