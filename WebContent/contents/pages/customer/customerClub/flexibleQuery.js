...
//						   forceFit:true,
	...
	var searchIndex = new Com.yucheng.crm.common.IndexField({ 
					...
					id:'searchIndex',
					name:'searchIndex',
					hiddenName:'searchIndex1',
					labelStyle: 'text-align:right;',
					singleSelect:true,
					anchor : '90%',
					callback :function(a,b,c,d){
					var mgr_namess = null;
					records1 = tarDictListPanel.getSelectionModel().selection;
					var mgrIds1 = '';
					mgr_namess = Ext.getCmp('searchIndex').getValue();
					if (mgr_namess != null && mgr_namess != ''){
		            partitionRuleStore.getAt(rowNo).data.indexId =this.ID;
		            partitionRuleStore.getAt(rowNo).data.indexName =this.NAME;
		            tarDictListPanel.getView().refresh(false);
						}
						}
					});
...
//				                           json6.CONNECTOR.push(temp.data.connector);
    ...
    var fiexibleEditPanel = new Ext.Panel( {
        ...
        tarDictListPanel.on('cellclick',function(grid,row,col){//获取编辑的行数，从0开始，
				rowNo=row;
			});