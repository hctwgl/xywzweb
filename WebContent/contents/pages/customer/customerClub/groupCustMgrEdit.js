	  ...
	  var addGroupCustMgr = function(addGrid){
					...
					data : [
                          {USER_ID:selectRe.data.userId,
                    	   ...
                    	   CUST_MANAGER_CONTACT:selectRe.data.mobilephone,
...
var searchGroupCustMgrPanel = new Ext.form.FormPanel({
	...
	var groupCustMgrStore = new Ext.data.Store({//数据存储
		  	restful:true,
		  	proxy : new Ext.data.HttpProxy({url:basepath+'/custmgrgroupinfo.json?querySign='+'queryCustomer'
		  	}),
		  	  reader: new Ext.data.JsonReader({
		      ...
		      ,{name: 'mobilephone', mapping: 'CUST_MANAGER_CONTACT'}
	...
	var groupCustMgrGrid = new Ext.grid.EditorGridPanel({
		...
		tbar : [{
			...
			Ext.Ajax.request({
                    ...
                    params:{
                        ...
                        'groupBaseId':editGroupBaseInfoForm.form.findField('id').getValue()
                    },
                    success : function() {
	 ...
	 //客户群维护窗口展示的from
    var groupCustMgrPanel = new Ext.Panel( {
        layout : 'fit',
        id:'info3',
        activeItem : 0,     
        autoScroll : true,
