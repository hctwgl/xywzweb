						...
						items : [
									{
										...
										handler : function() {											
											...
											//Ext.getCmp('gdsSrcOra').setValue('国阳');
											outStore.reload();
											var confirmSend = qForm.getForm().findField('confirmSend').getValue();
											if (confirmSend == '1'){
												Ext.Msg.alert('提示','该发运通知已确认发货，不允许再添加商品!');
												return false;
											}
											addXywzLogiDelvMerchdWindow.show();
										}
									},
									'-',
									{
										text : '<font color=red>修改包装</font>',
										iconCls : 'editIconCss',
										handler : function() {
											...
											if (selectLength != 1) {
												...
												editXywzLogiDelvMerchdForm.getForm().loadRecord(selectRe);
												editXywzLogiDelvMerchdWindow.show();
									...
									'-',
									{
										...
										handler : function() {
											var selectLength = grid.getSelectionModel().getSelections().length;
											var confirmSend = grid.getSelectionModel().getSelections()[0].data.confirmSend;
											if (confirmSend == '1'){
												Ext.Msg.alert('提示','不能删除确认发货的商品!');
												return;
											}
											if (selectLength < 1) {
											...
											for ( var i = 0; i < selectLength; i++) {
												...
												tmpOutId = selectRe.data.outId;
												tmpJianCnt = selectRe.data.qty;
												tmpRemZhi=selectRe.data.remZhiCnt;
												temWeight1=selectRe.data.weight;
												tmpTotalZhi=selectRe.data.totalZhi;
												idStr += tempId;
												outId += tmpOutId;
												jianCnt +=tmpJianCnt;
												remZhi+=tmpRemZhi;
												weight1+=temWeight1;
												totalZhi+=tmpTotalZhi;
												if (i != selectLength - 1)
													idStr += ',';
												    outId += ',';
												    jianCnt+=',';
											...
											//Ext.Msg.alert(idStr+':'+outId+':'+jianCnt );
												Ext.Ajax.request({
														url : basepath+ '/XywzLogiDelvMerchdAction!batchDestroy.json?idStr='+ idStr+'&outId='+outId+'&jianCnt='+jianCnt+'&remZhi='+remZhi+'&weight1='+weight1+'&totalZhi='+totalZhi,
													    //url : basepath+ '/XywzLogiDelvMerchdAction!batchDestroy.json',
													    //method : 'POST',
//														params:{
//															'idStr':idStr,
//															'outId':outId,
							...
							buttons : [ {
								...
								handler : function() {
									...
									for ( var i = 0; i < selectLength; i++) {
										...
										jianCntTmp=parseInt(selectRe.data.jianCnt1); //修改之前的原始件数值
										remZhiTmp=selectRe.data.remZhiCnt1;//修改之前的原始零支值
										...
										idStr += tempId;
										gdsSrc += gdsId;
										jianCnt+=parseInt(jianCnt1);
										zhiCnt+=parseInt(zhiCnt1);
										var totalZhi=parseInt(zhiCnt1)*parseInt(jianCnt1)+parseInt(remZhiCnt);
										if(null!=llWeight&&llWeight!=''){
											weight=(parseFloat(llWeight)*parseFloat(len)* parseInt(totalZhi))/1000.000;
										}
										//alert(weight);
										if ( jianCnt1>jianCntTmp||jianCnt1 <1 ){
											Ext.Msg.alert('提示','修改的件数不能大于原始值：'+jianCntTmp);
											//listPanel.form.findField().setValue();
											//selectRe.getCmp("jianCnt123").setValue(jianCntTmp);
											return false;
										}
										//alert("zhiCnt1:"+zhiCnt1);
										//alert("remZhiCnt:"+remZhiCnt);
										if ( parseInt(remZhiCnt)>parseInt(zhiCnt1)||parseInt(remZhiCnt) <0 ){
											Ext.Msg.alert('提示','修改的零支不能大于每件支数：'+remZhiTmp);
											return false;
										}
										//return;
										tempWeight+=weight;
										tmpRemZhiCnt+=parseInt(remZhiCnt);
