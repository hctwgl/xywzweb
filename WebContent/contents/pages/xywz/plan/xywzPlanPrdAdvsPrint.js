...
//						var issueDt = qForm.getForm().findField('ISSUES_DT').getValue();
						var planNu = qForm.getForm().findField('PLAN_NUM').getValue();
...
//						if(issueDt == null ||issueDt==''|| planNu == null||planNu == ''){
			...
			var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
					...
					url : basepath + '/XywzPlanJobAdvsSnglQueryAction.json'
				}),
				reader : new Ext.data.JsonReader({
			...
			var tbar = new Ext.Toolbar(
					{
						items : [
//						         {
//							text : '<font color=red>取消排产计划 </font>',
...
//						},'-',
									{
										...
										handler : function() {
											...
											var planNum = qForm.getForm().findField('PLAN_NUM').getValue();
//											if(issuesDt==null||issuesDt==''||planNum==null ||planNum=='' )
											taskInfoForm.getForm().reset();
											taskInfoWindow.show();
											//var issuesDt=taskInfoForm.getForm().findField('issueDate').getValue();
											//alert(issuesDt);
											//return;
