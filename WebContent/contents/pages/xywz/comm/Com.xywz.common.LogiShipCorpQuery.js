 ...
 * @since 2015-10-11
 */
 Com.xywz.common.LogiShipCorpQuery = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onRender : function(ct, position){
    	Com.xywz.common.LogiShipCorpQuery.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
    ...
    onTrigger2Click : function(){
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    {header : '船务公司ID',dataIndex : 'SHIP_CORP_ID',sortable : true,width : 150},
    	    {header : '船务公司名称',dataIndex : 'CORP_NM',width : 200,sortable : true},
    	    {header : '联系人',dataIndex : 'CONTCR',width : 200,sortable : true},
    	    {header : '联系人电话',dataIndex : 'CONT_TEL1',width : 200,sortable : true}
    	]);
    	/**
    	 * 数据存储
    	 */
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/XywzLogiShipCorpMgmtQueryAction.json'}),
    				...
    				if(!(_this.oCustomerQueryGrid.getSelectionModel().selections==null)){
						...
						for(var i=0;i<checkedNodes.length;i++){
							if(i==0){
								...
								oThisSearchField.hiddenField.setValue(checkedNodes[i].data.SHIP_CORP_ID);
							}else{
								sName=sName+','+checkedNodes[i].data.CORP_NM;
								oThisSearchField.hiddenField.setValue(_this.hiddenField.value+','+checkedNodes[i].data.SHIP_CORP_ID);
							}
						}
						oThisSearchField.setRawValue(sName);
						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
							oThisSearchField.shipCorpId=checkedNodes[0].data.SHIP_CORP_ID;
							oThisSearchField.shipCorpName=checkedNodes[0].data.CORP_NM;
							oThisSearchField.contcr=checkedNodes[0].data.CONTCR;
							oThisSearchField.contTel1=checkedNodes[0].data.CONT_TEL1;
						}
					}
				}
...
Ext.reg('customerquery',Com.xywz.common.LogiShipCorpQuery);