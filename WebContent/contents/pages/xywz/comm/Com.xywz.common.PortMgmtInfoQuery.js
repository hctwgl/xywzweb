 ...
 * @since 2012-11-08
 */
 Com.xywz.common.PortMgmtInfoQuery = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onRender : function(ct, position){
    	Com.xywz.common.PortMgmtInfoQuery.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
...
//    linkUser:'',//客户联系人,wzy,add,20130227
    ...
    onTrigger2Click : function(){
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    {header : '港口ID',dataIndex : 'PORT_ID',sortable : true,width : 100},                                
    	    {header : '港口中文名称',dataIndex : 'PORT_NAME_CN',sortable : true,width : 100},
    	    {header : '港口英文名称',dataIndex : 'PORT_NAME_EN',sortable : true,width : 100}
    	]);
    	/**
    	 * 数据存储
    	 */
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/XywzLogiPortInfoMgmtQueryAction.json'}),
    			...
    			handler:function(){
    				...
    				if(!(_this.oCustomerQueryGrid.getSelectionModel().selections==null)){
						...
						for(var i=0;i<checkedNodes.length;i++){
							if(i==0){
								sName=checkedNodes[i].data.PORT_NAME_CN;
								oThisSearchField.hiddenField.setValue(checkedNodes[i].data.PORT_ID);
							}else{
								sName=sName+','+checkedNodes[i].data.PORT_NAME_CN;
								oThisSearchField.hiddenField.setValue(_this.hiddenField.value+','+checkedNodes[i].data.PORT_ID);
							}
						}
						oThisSearchField.setRawValue(sName);
						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
							oThisSearchField.portId=checkedNodes[0].data.PORT_ID;
							oThisSearchField.portNmCn=checkedNodes[0].data.PORT_NAME_CN;
							oThisSearchField.portNmEn=checkedNodes[0].data.PORT_NAME_EN;
						}
					}
				}
...
Ext.reg('customerquery',Com.xywz.common.PortMgmtInfoQuery);