 ...
 * @since 2012-11-08
 */
 Com.xywz.common.InvMerchdDtlQuery = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onRender : function(ct, position){
    	Com.xywz.common.InvMerchdDtlQuery.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
...
//    linkUser:'',//客户联系人,wzy,add,20130227
    ...
    onTrigger2Click : function(){
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    {header : '合同号',dataIndex : 'CONTR_NUM',sortable : true,width : 100},                                
    	    {header : '发票号',dataIndex : 'INV_NUM',sortable : true,width : 100},
    	    {header : '品名',dataIndex : 'HS_CODE',sortable : true,width : 100},
    	    {header : '规格型号',dataIndex : 'MODEL',width : 100,sortable : true},
    	    {header : '吨数',dataIndex : 'QTY',width : 150,sortable : true},
    	    {header : '单价',dataIndex : 'AMT',width : 150,sortable : true},
    	    {header : '材质',dataIndex : 'MATERIALS',width : 150,sortable : true}   	    
    	]);
    	/**
    	 * 数据存储
    	 */
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/XywzInvMerchdDtlQueryAction.json'}),
    				...
    				if(!(_this.oCustomerQueryGrid.getSelectionModel().selections==null)){
						...
						for(var i=0;i<checkedNodes.length;i++){
							if(i==0){
								...
								oThisSearchField.hiddenField.setValue(checkedNodes[i].data.MODEL);
							}else{
								sName=sName+','+checkedNodes[i].data.INV_NUM;
								oThisSearchField.hiddenField.setValue(_this.hiddenField.value+','+checkedNodes[i].data.MODEL);
							}
						}
						...
						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
							...
							oThisSearchField.model=checkedNodes[0].data.MODEL;
						}
					}
				}
...
Ext.reg('customerquery',Com.xywz.common.InvMerchdDtlQuery);