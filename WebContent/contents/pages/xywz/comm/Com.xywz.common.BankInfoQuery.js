 ...
 * @since 2012-11-08
 */
 Com.xywz.common.BankInfoQuery = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onRender : function(ct, position){
    	Com.xywz.common.BankInfoQuery.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
...
//    linkUser:'',//客户联系人,wzy,add,20130227
    ...
    onTrigger2Click : function(){
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    {header : '银行ID',dataIndex : 'BANK_ID',sortable : true,width : 100},                                
    	    {header : '银行全称',dataIndex : 'BANK_FULL_NM',sortable : true,width : 100},
    	    {header : '银行地址',dataIndex : 'BANK_ADDR',sortable : true,width : 100},
    	    {header : '银行电话',dataIndex : 'BANK_TEL',width : 100,sortable : true},
    	    {header : 'SWIFTCODE',dataIndex : 'SWIFT_CODE',width : 150,sortable : true},
    	    {header : '我方账户',dataIndex : 'ACCOUNT',width : 150,sortable : true},
    	    {header : '备注',dataIndex : 'MEMO',width : 150,sortable : true}
    	]);
    	/**
    	 * 数据存储
    	 */
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/XywzParaBankQueryAction.json'}),
    		reader: new Ext.data.JsonReader({
    			...
    			handler:function(){
    				...
    				if(!(_this.oCustomerQueryGrid.getSelectionModel().selections==null)){
						...
						for(var i=0;i<checkedNodes.length;i++){
							if(i==0){
								sName=checkedNodes[i].data.BANK_FULL_NM;
								oThisSearchField.hiddenField.setValue(checkedNodes[i].data.BANK_ID);
							}else{
								sName=sName+','+checkedNodes[i].data.BANK_FULL_NM;
								oThisSearchField.hiddenField.setValue(_this.hiddenField.value+','+checkedNodes[i].data.BANK_ID);
							}
						}
						oThisSearchField.setRawValue(sName);
						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
							oThisSearchField.bankFullNm=checkedNodes[0].data.BANK_FULL_NM;
							oThisSearchField.bankId=checkedNodes[0].data.BANK_ID;
							oThisSearchField.bankAddr=checkedNodes[0].data.BANK_ADDR;
							oThisSearchField.bankTel=checkedNodes[0].data.BANK_TEL;
							oThisSearchField.swiftCode=checkedNodes[0].data.SWIFT_CODE;
							oThisSearchField.account=checkedNodes[0].data.ACCOUNT;
							oThisSearchField.memo=checkedNodes[0].data.MEMO;
						}
					}
				}
...
Ext.reg('customerquery',Com.xywz.common.BankInfoQuery);