 ...
 * @since 2012-11-08
 */
 Com.xywz.common.CustomerInfoQuery = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onRender : function(ct, position){
    	Com.xywz.common.CustomerInfoQuery.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
...
//					_this.oCustomerQueryForm.getForm().findField('CUST_ORG').setValue('');
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    ...
    	    {header : '客户等级',dataIndex : 'CUST_LVL_ORA',width : 150,sortable : true},
    	    {header : '信用等级',dataIndex : 'CRDT_LVL_ORA',width : 80,sortable : true},
    	...
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/XywzCustCustInfoQueryAction.json'}),
    		reader: new Ext.data.JsonReader({
    			...
    			handler:function(){
    				...
    				if(!(_this.oCustomerQueryGrid.getSelectionModel().selections==null)){
						...
						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
							...
							oThisSearchField.custName=checkedNodes[0].data.CUST_SHT_NM;
...
Ext.reg('customerquery',Com.xywz.common.CustomerInfoQuery);