...
//						editable : false,
	...
	// 复选框
					...
					items : [
							{
								...
								iconCls : 'addIconCss',
								handler : function() {
									addXywzAsstMachgMgmtBankForm.getForm()
											.reset();
									addXywzAsstMachgMgmtBankWindow.show();
								}
							},
							'-',
							{
								...
								handler : function() {
									...
									if (selectLength != 1) {
										...
										editXywzAsstMachgMgmtBankForm.getForm()
												.loadRecord(selectRe);
										editXywzAsstMachgMgmtBankWindow.show();
														...
														function(buttonId) {
															...
															for ( var i = 0; i < selectLength; i++) {
																selectRe = grid
																		.getSelectionModel()
																		.getSelections()[i];
																tempId = selectRe.data.id; //删除的ID需要修改
																idStr += tempId;
																if (i != selectLength - 1)
																	idStr += ',';
															}
															Ext.Ajax
																	.request( {
																		url : basepath
																				+ '/XywzAsstMachgMgmtBankAction!batchDestroy.json?idStr='
																				+ idStr,
					        ...
					        }),'-',
							{
								...
								handler : function() {
									...
									if (selectLength != 1) {
										...
										detailXywzAsstMachgMgmtBankForm
												.getForm().loadRecord(
														selectRe);
										detailXywzAsstMachgMgmtBankWindow.show();
									}
								}
							} ]
				});
		...
		// 新增窗口展示的from
		var addXywzAsstMachgMgmtBankForm = new Ext.form.FormPanel(
				{
					labelWidth : 150,
					height : 150,
					frame : true,
								...
								items : [
											...
											items : [ new Com.xywz.common.BankInfoQuery(
													{
														...
														callback : function(a, b) {
															...
															addXywzAsstMachgMgmtBankForm.getForm().findField('bankAddr').setValue(records[0].data.BANK_ADDR);
															addXywzAsstMachgMgmtBankForm.getForm().findField('bankTel').setValue(records[0].data.BANK_TEL);
														}
													}) ]
										},
										{
										...
										},
										{
											...
											items : [ {
												...
												Width : '100',
												name : 'acctNum',
												fieldLabel : '<font color=red>*</font>账户',
												allowBlank : false,
												blankText : '账户不能为空',
												maxLength : 50,
												minLength : 1,
								...
								buttons : [
										{
											text : '保  存',
											handler : function() {
												if (!addXywzAsstMachgMgmtBankForm
														.getForm().isValid()) {
													Ext.Msg.alert('提示',
															'输入格式有误，请重新输入!');
													return false;
												...
												Ext.Ajax
														.request( {
															url : basepath + '/XywzAsstMachgMgmtBankAction.json',
															method : 'POST',
															form : addXywzAsstMachgMgmtBankForm
																	.getForm().id,
															waitMsg : '正在保存数据,请等待...', // 显示读盘的动画效果，执行完成后效果消失
															success : function(
																	response) {
												...
												addXywzAsstMachgMgmtBankWindow.hide();
											}
										}, {
											text : '取  消',
											handler : function() {
		...
		// 修改窗口展示的from
		var editXywzAsstMachgMgmtBankForm = new Ext.form.FormPanel(
				{
					labelWidth : 150,
					height : 150,
					frame : true,
								...
								buttons : [
										{
											text : '保  存',
											handler : function() {
												if (!editXywzAsstMachgMgmtBankForm
														.getForm().isValid()) {
													Ext.Msg.alert('提示',
															'输入格式有误，请重新输入!');
													return false;
												...
												Ext.Ajax
														.request( {
															url : basepath + '/XywzAsstMachgMgmtBankAction.json',
															method : 'POST',
															form : editXywzAsstMachgMgmtBankForm
																	.getForm().id,
															waitMsg : '正在保存数据,请等待...', // 显示读盘的动画效果，执行完成后效果消失
															success : function(
																	response) {
												...
												editXywzAsstMachgMgmtBankWindow
														.hide();
											}
										},
										{
		...
		// 预览展示的from
		var detailXywzAsstMachgMgmtBankForm = new Ext.form.FormPanel({
			labelWidth : 150,
			height : 150,
			frame : true,
			region : 'center',
			...
			items : [ {
				...
				buttons : [
					...
					text : '返  回',
					handler : function() {
				    	detailXywzAsstMachgMgmtBankWindow.hide();
					}
				} ]
			}
			]
		...
		// 定义新增窗口
		var addXywzAsstMachgMgmtBankWindow = new Ext.Window( {
			title : '外协加工银行信息新增',
			plain : true,
			layout : 'fit',
			width : 800,
			...
			buttonAlign : 'right',
			border : false,
			items : [ addXywzAsstMachgMgmtBankForm ]
		});

		// 定义修改窗口
		var editXywzAsstMachgMgmtBankWindow = new Ext.Window( {
			title : '外协加工银行信息修改',
			plain : true,
			layout : 'fit',
			width : 880,
			...
			titleCollapse : true,
			border : false,
			items : [ editXywzAsstMachgMgmtBankForm ]
		});
		
		// 定义详情窗口
		var detailXywzAsstMachgMgmtBankWindow = new Ext.Window({
			title : '外协加工银行信息预览',
			plain : true,
			layout : 'fit',
			width : 880,
			...
			titleCollapse : true,
			border : false,
			items : [ detailXywzAsstMachgMgmtBankForm ]
		});
