...
viewConfig: {
	...
	onCalendarEndDrag : function(start, end, onComplete){
        ...
        if(this.fireEvent('rangeselect', this, dates, onComplete) !== false){
            this.showEventEditorDrag(dates, null);
...
alert('End: '+bounds.end);
                ...
                listeners: {
                    'eventadd': {
                        fn: function(win, rec, animTarget){
                            ...
                            if(Ext.getCmp('ext_cal_fullitem_mynotic')){
                    ...
                    'editdetails': {
                        fn: function(win, rec, animTarget, view){
                            win.hide(animTarget);
                            if(Ext.getCmp('ext_cal_fullitemwin')){
								Ext.getCmp('ext_cal_fullitemwin').hide();                            
                            }
    ...
    /**
     * 
     * create by caiqy in 2011-11-29
     * 
     */
    getFullItemsWindow : function(basePath,currentDate,calStoreArray,eventEditWin,
    	rec,animateTarget,target){
    	//分页 limit 12
    	var pageSize = 12;
    		    	
	    	var getPagingToolbar = function(store){
	    		var pagingToolBar = new Ext.PagingToolbar({
	    			width:document.body.clientWidth-420,
					pageSize: pageSize,
					store: store,
					displayInfo: true,
        ...
        {name: 'topActiveDate', mapping: 'TOP_ACTIVE_DATE', type: 'date'},  
		...
		var publicNoticeReader = new Ext.data.JsonReader({
			successProperty:'success',
		...
		var publicNoticeGridStore = new Ext.data.Store({
			restful:true,
			autoLoad:true,
			...
			baseParams:{
				cal_curdate:currentDate.format('Y-m-d'),
				limit:pageSize
			},
			reader:publicNoticeReader
		});
		...
		var pagesize_combo = new Ext.form.ComboBox({
        	...
        	store : new Ext.data.ArrayStore({
            	fields : ['value', 'text'],
            	data : [[100, '100条/页'], [250, '250条/页'], [500, '500条/页'],[1000,'1000条/页']]
        	}),
       		valueField : 'value',
        	displayField : 'text',
        	value: 100,
    	...
    	var publicNoticeGrid = new Ext.grid.GridPanel({
    		id:'ext_cal_fullitem_publicnotic',
    	...
    	/*信息提醒-结束*/
    	/*我的提醒-开始*/
    	var myNoticeRecord = Ext.data.Record.create([
    		//primary key
    		{name:'SCHEDULE_ID',mapping:'SCHEDULE_ID'},
    		//提醒标题
    		{name:'SCHEDULE_TITLE',mapping:'SCHEDULE_TITLE'},
    		//开始日期
    		...
    		//提醒内容
    		{name:'SCHEDULE_CONTENT',mapping:'SCHEDULE_CONTENT'},
    		//提醒类别
    		{name:'REMIND_BELONG',mapping:'REMIND_BELONG'},
    		//提醒类别名称
    		{name:'REMIND_BELONG_NAME',mapping:'REMIND_BELONG_NAME'}
    	]);
    	
    	var myNoticeReader = new Ext.data.JsonReader({
    		successProperty : 'success',
		...
		var colModel = new Ext.grid.ColumnModel([sm,
			{header:'提醒标题',dataIndex:'scheduleTitle',width:80,align:'center'},
		  	{header:'提醒类别',dataIndex:'remindBelongName',width:60,align:'center'},
		  	{header:'开始日期',dataIndex:'startDate',width:80,align:'center'},
		  	{header:'结束日期',dataIndex:'endDate',width:80,align:'center'},
		  	{header:'内容',dataIndex:'scheduleContent',width:120,align:'center'}
		]);
		var myNoticeStore = new Ext.data.ArrayStore({
			fields:[{name:'scheduleId'},
					{name:'scheduleTitle'},
					{name:'remindBelongName'},
					{name:'remindBelong'},
					{name:'startDate'},
					{name:'endDate'},
					{name:'scheduleContent'}],
			ownLoad:function(targetStore){
				var sourceArray = new Array();
				if(targetStore){
					targetStore.each(function(e){
						if(e.get('StartDt').format('Ymd') <= currentDate.format('Ymd') 
							&& e.get('EndDt').format('Ymd') >= currentDate.format('Ymd')){
							var ss = new Array();
							ss.push(e.get('ID'));
							ss.push(e.get('EvtTitle'));
							ss.push(calStoreArray[e.get('CalID')]==""
    								?e.get('CalID')
    								:calStoreArray[e.get('CalID')]);
							ss.push(e.get('CalID'));
							ss.push(e.get('StartDt'));
							ss.push(e.get('EndDt'));
							ss.push(e.get('Desc'));
					...
					this.removeAll();
					this.loadData(sourceArray);
					if(Ext.getCmp('ext_cal_fullitem_mynotic')){
						Ext.getCmp('ext_cal_fullitem_mynotic').reconfigure(this,colModel);
					}
				}
			}
		});
		myNoticeStore.ownLoad(target.store);
    	
    	var myNoticeGrid = new Ext.grid.GridPanel({
    		id:'ext_cal_fullitem_mynotic',
    		width:document.body.clientWidth-420,
    		height:	390,
    		store:myNoticeStore,
    		sm:sm,
			...
			tbar:[
				{
				 text:'新增'
				,handler:function(){
					eventEditWin.show(rec,animateTarget,target);
				}},{
				 text:'修改'
				,handler:function(){
					var columns = myNoticeGrid.getSelectionModel().getSelections();
					if(columns.length == 1){
						var columnId = columns[0].data.scheduleId;
						var recClick = target.getEventRecord(columnId);
						eventEditWin.show(recClick,animateTarget,target);
					}else{
				...
				,handler:function(){
					var columns = myNoticeGrid.getSelectionModel().getSelections();
					if(columns.length <= 0){
						Ext.Msg.alert('提示','请选择一条或多条记录进行删除');
						return ;
					}
					Ext.MessageBox.confirm('提示','确定删除吗?',function(){						
						...
						for(var count=0;count<columns.length;count++){
							var delModule = Ext.getCmp(target.id).getEventRecord(columns[count].data.scheduleId);
							target.onEventDelete(null, delModule);
							idArray.push(columns[count].data.scheduleId);					
						}
						Ext.getCmp('ext_cal_fullitem_mynotic').getStore().each(function(rec){
							if(idArray.indexOf(rec.data.scheduleId) != -1){
								Ext.getCmp('ext_cal_fullitem_mynotic').getStore().remove(rec);
							}
						});
					});
				}}
...
//			,
//			bbar:getPagingToolbar(myNoticeStore)
    	});
    	/*我的提醒-结束*/
    	
    	var fullItemsTab = new Ext.TabPanel({
    		id:'ext_cal_fullitemtab',
    		width:document.body.clientWidth-400,
    		height:	450,
    		activeTab:0,
    		frame:true,
    	...
    	var fullItemsWin = new Ext.Window({
    		id:'ext_cal_fullitemwin',
    		width:document.body.clientWidth-400,
    		height:	450,
    		closable :true,
    		closeAction: 'close',
    ...
    /**
     * 
     * edit by caiqy in 2011.11.29
     * show a window contain 1.public notice 2.info notice 3.my notice
     * 
     */
    showEventEditor : function(rec, animateTarget,curDate){

    	if(typeof(rec.data) != 'undefined'){
        	this.getEventEditor().show(rec, animateTarget, this);
    	}else{
    		var calStoreArray = new Array();
    		//CalTitle,ID
    		var i ;
    		if(this.calendarStore){
    			this.calendarStore.each(function(sim){
    				calStoreArray[sim.data.ID] = sim.data.CalTitle;
    			});
    		}
    		this.getEventEditor();
    		if(Ext.getCmp('ext_cal_fullitemwin')){
    			Ext.getCmp('ext_cal_fullitemwin').close();
    		}
    		this.getFullItemsWindow(this.ownerCalendarPanel.contextPath
    			,curDate,calStoreArray
    			,this.getEventEditor()
    			,rec
    			,animateTarget
    			,this
    ...
    /*drag drop use only*/
    showEventEditorDrag : function(rec, animateTarget){
    	this.getEventEditor().show(rec, animateTarget, this);
        return this;
    },
    ...
    onDayClick: function(dt, ad, el){
        ...
        if(this.fireEvent('dayclick', this, dt, ad, el) !== false){
            ...
            this.showEventEditor(data, el,dt);
        }
    },
...
Ext.ensible.cal.CalendarPanel = Ext.extend(Ext.Panel, {
    ...
    onChangeDate: function(date){
    	if(date !== ''){
    		...
    		var calDate = new Date(dArray[0],dArray[1]-1,dArray[2]);
    		this.startDate = this.layout.activeItem.moveTo(calDate, true);
            this.updateNavState();
            // TODO: check that view actually changed:
            this.fireViewChange();
    	}
