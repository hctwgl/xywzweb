...
@Action(value="/XywzPlanPrdMerchdListQueryAction", results={
    @Result(name="success", type="json")
})
public class XywzPlanPrdMerchdListQueryAction extends BaseQueryAction {
	@Autowired
	@Qualifier("dsOracle")
	...
	public void prepare() {
    	...
    	StringBuilder sb=new StringBuilder("SELECT T.* FROM (SELECT  "
												...
												+"	INNER JOIN XYWZ_PLAN_PRDC_PLAN_ADVS_SNGL S"
												+"	ON (T.CONTR_NUM=S.CONTR_NUM AND T.HS_CODE=S.PRD_NAME AND T.SPC_MODEL=S.SPC_MODEL AND T.LEN=S.LEN AND S.SCHEDU_STATUS ='3')"
												+"	WHERE 1=1                                "
    											);    	
