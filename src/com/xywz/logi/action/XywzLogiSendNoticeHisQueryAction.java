...
@Action(value="/XywzLogiSendNoticeHisQueryAction", results={
    @Result(name="success", type="json")
})
public class XywzLogiSendNoticeHisQueryAction extends BaseQueryAction {
	@Autowired
	@Qualifier("dsOracle")
	...
	public void prepare() {
    	...
    	StringBuilder sb=new StringBuilder("select t.*,T1.PORT_NAME_CN AS PORT_NAME_CN, T2.PORT_NAME_CN AS UN_PORT_NAME_CN,T3.CORP_NM,T4.AGENT_NAMR,CST.CUST_SHT_NM " +
    			"from XYWZ_LOGI_SEND_NOTICE_HIS t " +
        ...
        for(String key:this.getJson().keySet()){
            if(null!=this.getJson().get(key)&&!this.getJson().get(key).equals("")){
                ...
                }else if (key.equals("mkTabDt")){
                	sb.append(" and MK_TAB_DT = "+this.getJson().get(key));                	
                }    
            }
        }
