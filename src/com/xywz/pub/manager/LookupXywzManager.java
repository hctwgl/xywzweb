 ...
 *         增加注释
 */
public class LookupXywzManager {
	
	/***单态对象*/
	private static LookupXywzManager instance;
	/***日志*/
	private static Logger log = Logger.getLogger(LookupXywzManager.class);
	/***取dataSource*/
	...
	/***取单态方法*/
	public static synchronized LookupXywzManager getInstance() {
		if (instance != null) {
			return instance;
		} else {
			instance = new LookupXywzManager();
		}
		return instance;
	}
	/***设置数据字典类别*/
	...
	private int loadOracleLookup(String name) {
		String SQL = "SELECT t2.f_name, t1.f_code, t1.f_value FROM xywz_sys_lookup_item t1 LEFT JOIN xywz_sys_lookup t2 ON t1.f_lookup_id = t2.f_name";
		if (name != null) {
			SQL += " WHERE t2.f_name = ?";
		}
		int rowsCached = 0;
...
//		log.info("完成加载菜单映射数据,共计:" + count2);
	...
	private Cache getCache(String name) {
    	return EhCacheManager.getInstance().getCacheManager().getCache(name);
    }
	/***
