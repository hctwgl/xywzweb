...
@Action("/contributionInformation-info")
	public class ContributionInformationAction extends BaseQueryAction {
		...
		public void prepare() {
			...
			StringBuilder sb = new StringBuilder("select t.* from acrm_f_ci_cust_contribution t where 1>0 ");
			ActionContext ctx = ActionContext.getContext();
	        request = (HttpServletRequest)ctx.get(StrutsStatics.HTTP_REQUEST);
	        if(request.getParameter("cust_id")!=null){
	        	String custStr = request.getParameter("cust_id");
	        	if("G".equals(custStr.substring(0, 1))){//再通过客户群查询客户的贡献度信息时调用
	        		String groupStr [] = custStr.split(",");
	        		sb.append("and t.custom_id in(select cust_id from OCRM_F_CI_RELATE_CUST_BASE t where t.cust_base_id='"+groupStr[1]+"')");
	        	}else{
					sb.append(" and t.custom_id = '"+request.getParameter("cust_id")+"'"  );
	        	}
			}
