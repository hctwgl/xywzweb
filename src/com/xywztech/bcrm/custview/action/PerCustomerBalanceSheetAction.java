...
import com.xywztech.bcrm.custview.service.PerCustomerBalanceSheetService;
import com.xywztech.bob.common.CommonAction;

@SuppressWarnings("serial")
@Action("/perCustomerBalanceSheet-info")
public class  PerCustomerBalanceSheetAction extends CommonAction{
    @Autowired
    private PerCustomerBalanceSheetService perCustomerBalanceSheetService ;
    @Autowired
	public void init(){
	  	model = new AcrmFCiCbalanceSheet(); 
		setCommonService(perCustomerBalanceSheetService);
  ...
  //（自定义）批量删除
	...
	//system.out.printlnntln("-----------"+idStr+"----------------");
			String jql="delete from AcrmFCiCbalanceSheet c where c.id in ("+idStr+")";
			Map<String,Object> values=new HashMap<String,Object>();
			perCustomerBalanceSheetService.batchUpdateByName(jql, values);
...
//          Long jgdm=auth.getXtUnitId();
    	StringBuilder sb=new StringBuilder("select c from AcrmFCiCbalanceSheet c where 1=1 ");    	
    	Map<String,Object> values=new HashMap<String,Object>();
    	ActionContext ctx = ActionContext.getContext();
        request = (HttpServletRequest)ctx.get(StrutsStatics.HTTP_REQUEST);
