...
@Action(value = "/queryseniorquery", results = { @Result(name = "success", type = "json"), })
public class QuerySeniorQueryAction {
	@Autowired
	@Qualifier("dsOracle")
	...
	public String index() {
		...
		try {
			StringBuilder sb = new StringBuilder(
					"SELECT id AS nodeid,name,parent_id,'1' AS tables,'' AS ename, value,'' AS ctype,'' AS NOTES FROM MTOOL_DBTABLE  where parent_id <>'1' and id<>'1' UNION SELECT id AS nodeid,col_name_c AS name,dbtable_id AS parent_id,'2' AS tables,col_name_e AS ename, '' AS value,COL_TYPE AS ctype,NOTES FROM MTOOL_DBCOL WHERE IS_ENABLE='true'");
