...
import com.xywztech.bcrm.customer.service.BadnessCustomerService;
import com.xywztech.bob.common.CommonAction;

@SuppressWarnings("serial")
@Action("/badnessCustomer-info")
public class  BadnessCustomerAction extends CommonAction{
    @Autowired
    private BadnessCustomerService badnessCustomerService ;
    @Autowired
	public void init(){
	  	model = new OcrmFCiBadCustInfo(); 
		setCommonService(badnessCustomerService);
  ...
  //（自定义）批量删除
	...
	public String batchDestroy(){
    	try{
			...
			String jql="delete from OcrmFCiBadCustInfo c where c.custId in ('"+idStr.replaceAll(",","','")+"')";
			Map<String,Object> values=new HashMap<String,Object>();
			badnessCustomerService.batchUpdateByName(jql, values);
...
//          Long jgdm=auth.getXtUnitId();
    	StringBuilder sb=new StringBuilder("select c from OcrmFCiBadCustInfo c where 1=1 ");
    	Map<String,Object> values=new HashMap<String,Object>();
    	ActionContext ctx = ActionContext.getContext();
        request = (HttpServletRequest)ctx.get(StrutsStatics.HTTP_REQUEST);
