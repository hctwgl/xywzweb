...
@Action(value = "/querycustomerlinkman", results = { @Result(name = "success", type = "json") })
public class QueryCustomerLinkmanAction extends BaseQueryAction {
	...
	public void prepare(){
        ...
        StringBuilder sb = new StringBuilder(
                "select t.* "
                        + "from ocrm_f_ci_linkman t where t.CUST_ID= '"
                        + customerId + "'");
        for (String key : this.getJson().keySet()) {
            if (null != this.getJson().get(key)
                    && !this.getJson().get(key).equals("")) {
                if (key.equals("LK_NAME"))
                    ...
                    sb.append(" and t." + key + " like '%" + this.getJson().get(key)
                            + "%'");
                else if (key.equals("FZ_BUSINESSTYP"))
                    sb.append(" and t." + key + " like '%" + this.getJson().get(key)
                            + "%'");
